    ...
    class SplineShape2DEditor : Shape2DEditor
    {
            ...
            for (int i = 0; i < controlPointCount; i++)
            {
                ...
                if (!hoverHandle && (GUIUtility.hotControl == 0 || GUIUtility.hotControl == addHandleID) && (path.loop || i != 0))
                {
                    ...
                    if (HandleUtility.DistanceToPolyLine(s_Polyline) < 10)
                    {
                        ...
                        {
                            var fmh_256_72_638753853189271110 = Quaternion.identity; Handles.FreeMoveHandle(addHandleID, point, PathHandleUtility.GetHandleSize(point), Vector3.zero, Handles.SphereHandleCap);
                        }
                        if (EditorGUI.EndChangeCheck())
                        {
        ...
        static class SplineHandleUtility
        {
            ...
            public static Vector3 DoControlHandle(int id, Vector3 position)
            {
                ...
                var fmh_318_65_638753853189276080 = Quaternion.identity; position = Handles.FreeMoveHandle(id, position, handleSize, Vector3.zero, Handles.SphereHandleCap);
