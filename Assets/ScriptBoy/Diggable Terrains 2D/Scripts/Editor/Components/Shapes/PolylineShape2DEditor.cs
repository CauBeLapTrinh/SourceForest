        ...
        protected override void DrawHandles()
        {
            ...
            using (new Handles.DrawingScope(PathHandleUtility.handleColor))
            {
                ...
                for (int i = 0; i < n; i++)
                {
                    ...
                    EditorGUI.BeginChangeCheck();
                    var fmh_116_59_638753853188862720 = Quaternion.identity; pos = Handles.FreeMoveHandle(id, pos, size, Vector3.zero, Handles.SphereHandleCap);
                    if (EditorGUI.EndChangeCheck())
                    {
                    ...
                    if (!hoverHandle && (GUIUtility.hotControl == 0 || GUIUtility.hotControl == addId) && (path.loop || i != 0))
                    {
                        ...
                        if (HandleUtility.DistanceToLine(prevPos, pos) < 5)
                        {
                            ...
                            {
                                var fmh_137_80_638753853188867390 = Quaternion.identity; addPos = Handles.FreeMoveHandle(addId, addPos, size, Vector3.zero, Handles.SphereHandleCap);
                            }
                            if (EditorGUI.EndChangeCheck())
                            {
                                Undo.RecordObject(path, "Edit Polyline Path");
